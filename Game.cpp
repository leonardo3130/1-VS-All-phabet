...
Game::Game(char filePath[], int atk, int def, int livello) {
  ...
  int m_x, m_y, m_atk = 2 + (livello), m_def = 2 + (livello), m_mode,
                m_max_hp = 20 + (10 * (livello - 1));
  ...
  if (file) {
    ...
    for (int i = 0; i < this->map.getHeight(); i++) {
      for (int j = 0; j < this->map.getWidth(); j++) {
        ...
        if (this->map.isMonster(j, i) && !this->map.isTurret(j, i)) {
          ...
          Monster mostro(j, i, m_hp, m_atk, m_def, this->map.getMapChar(i, j),
                         0);
        ...
        // torre
          ...
          Monster mostro(j, i, t_hp, m_atk + 5, m_def + 5,
                         this->map.getMapChar(i, j), 1);
          this->lista_mostri = new_monster(this->lista_mostri, mostro);
          this->map.setMapChar(i, j, ' ');
          this->n_torri++;
        }
    ...
    for (int i = 0; i < this->n_mostri; i++) {
      ...
      if (map.isEmpty(m_x, m_y) && !map.isMoney(m_x, m_y) &&
          !map.isMonster(m_x, m_y)) {
...
int Game::run(Player &p) {
  ...
  // game loop
  int prev_x, prev_y, c = 0, b = 0, d = 0, e = 0, f = 0, tmp_x, tmp_y, tmp_dir,
                      tmp_ar;
  ...
  while (esito == IN_GAME || esito == WIN) {
    ...
    if (c == mon_speed) {
      ...
      while (tmp_m != NULL) {
        ...
        if (tmp_m->mon.getHp() <= 0) { // controllo hp
          ...
          p.incScore(
              50); // quando si uccide un mostro si prendono 50 punti aggiuntivi
    ...
    if (b == bul_speed) {
      ...
      while (tmp_b != NULL) {
        ...
        if (collision != 0) {
          ...
          if (collision ==
              2) { // se collision = 2 , il proiettile ha colpito il player
            if (this->lista_mostri != NULL)
              p.incHP(
                  -(double(lista_mostri->mon.getAtk()) / double(p.getDef())));
            else
              p.incHP(-1);
          }

          else if (collision ==
                   3) { // se collision = 3 , il proiettile ha colpito un mostro
            ...
            x = search_monster_by_xy(this->lista_mostri,
                                     (tmp_b->bul.getX()) + tmp_x,
                                     (tmp_b->bul.getY()) + tmp_y);
    ...
    if (d = 400) {
      ...
      for (int i = 0; i < 4; i++) {
        ...
        if (tmp_ar == 1) { // se nella posizione intorno al player c'è un mostro
          x = search_monster_by_xy(this->lista_mostri, (p.getX()) + tmp_x,
                                   (p.getY() + tmp_y));
          if (x != NULL) {
            if (!this->map.isTurret(x->mon.getX(), x->mon.getY())) {
    ...
    // Draw
    drawPlayer(game_win, map, p, prev_x, prev_y);
    wrefresh(game_win);
    ...
    if (esito != EXIT && esito != GO_TO_SHOP) {
      ...
      if (this->map.getCoins() == 0 && (this->map).protagonistInNextPortal())
        esito = GO_TO_NEXT;

      // il protagonista va al livello precedente
      if ((this->map).protagonistInPrevPortal() && p.getCurrentLevel() != 1)
        esito = GO_TO_PREV;
    }
  }
  this->map.clean();
...
pbul Game::handleInput(int c, Map &map, Player &giocatore, pbul bullet_list,
                       arnd around, int &e) {
  if (c == ERR) {
    ...
    case KEY_UP:
      if (around.above == 2) {
        ...
        giocatore.incScore(
            5); // la raccolta di 1 moneta porta anche 5 punti aggiuntivi
...
void Game::drawPlayer(WINDOW *win, Map &map, Player &protagonist, int prev_x,
                      int prev_y) {
  mvwprintw(win, prev_y, prev_x, " ");
  wrefresh(win);
  mvwprintw(win, protagonist.getY(), protagonist.getX(), "%c",
            protagonist.getLook());
  wrefresh(win);
}
...
void Game::drawMonster(WINDOW *win, Map &map, pmon monster_list) {
  pmon tmp = monster_list;
  while (tmp != NULL) {
    if (map.isInside(tmp->prev_x, tmp->prev_y) &&
        map.getMapChar(tmp->prev_y, tmp->prev_x) != '/' &&
        map.getMapChar(tmp->prev_y, tmp->prev_x) != '@')
...
void Game::drawBullet(WINDOW *win, Map &map, pbul bul_list) {
  pbul tmp3 = bul_list;
  while (tmp3 != NULL) {
    if (map.isInside(tmp3->prev_x, tmp3->prev_y) &&
        map.getMapChar(tmp3->prev_y, tmp3->prev_x) != '/' &&
        map.getMapChar(tmp3->prev_y, tmp3->prev_x) != '@')
...
void Game::drawStats(WINDOW *win, int x, int y, Player p) {
  box(win, 0, 0);
  mvwprintw(win, 0, 4, "Stats di %s - Lv. %d", p.getNick(),
            p.getCurrentLevel());
...
void Game::setMap(Map mappa) { this->map = mappa; }

pmon Game::getListaMostri() { return this->lista_mostri; }

void Game::setListaMostri(pmon l) { this->lista_mostri = l; }

int Game::getNMostri() { return this->n_mostri; }

void Game::setNMostri(int n) { this->n_mostri = n; }
