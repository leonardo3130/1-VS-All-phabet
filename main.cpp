...
ptr_livelli new_level(ptr_livelli l, Player &p) {
  ...
  if (l == NULL) {
    ...
    new_l->prev = l;
    new_l->partita =
        Game(filePath, p.getAtk(), p.getDef(), p.getCurrentLevel());
...
void shop(Player &p) {
  ...
  int starty = (LINES - 42) / 2, startx = (COLS - 82) / 2, highlights = 0,
      choice = 0;
  int prezzi[3] = {5, 5, 5};
  ...
  while (choice != 'q') {
    ...
    if (highlights == 0)
      wattron(shop, A_REVERSE);
    mvwprintw(shop, 27, 21, "+2 ATK");
    if (highlights == 0)
      wattroff(shop, A_REVERSE);
...
int main() {
  ...
  int choice =
      protagonist
          .choice_menu(); // 0;		//significa che il player esiste già,
                          // quindi deve solo prendere le statistiche
  ...
  if (choice == 0) {
    ...
    do {
      ...
      // genera la path del file da aprire:
      // Esempio Path nel caso giochi il player:"Andrea" e voglia salvare il
      // livello n:
      //  filePath = "Archivio/Andrea/Leveln.txt"
      ...
      if (protagonist.getCurrentLevel() == 1)
        head = gioco;
